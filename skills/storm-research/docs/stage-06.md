...
2. Assign each cluster to an executor per the routing rules in `docs/executors.md` (rules 1–5): the verifier is the first executor in rotation codex → agy → grok that is **not** that claim's actual producer and is not limited by usage caps — the producer-exclusion condition takes priority over rotation order. If an expert fell back to another executor, its claims are verified as that executor's output, not the originally assigned one's. grok joins the verification rotation only when confirmed (experimental, OFF by default — see `docs/executors.md`); until then, rotation is codex → agy. When no qualifying external executor exists, fall back to Claude fresh-context verification per rule 3. Dispatch clusters in parallel, then **print the dispatch frame below verbatim** (for non-Korean runs, render the same structure in the user's prompt language). The bar has **one segment per actual cluster** — clusters number 4–6, so never hard-code the bar width:
