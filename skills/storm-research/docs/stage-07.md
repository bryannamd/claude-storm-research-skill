...
2. Structure the terminal output with these sections:
   * Header: title, version (`V2`), date, expert count, executor line, reader role.
...
7. Record user approval or requested revisions before marking the run complete.
...
* [ ] Was the missing-sixth-expert V3 offer made, and user approval recorded?
* [ ] Was the `humanizer` skill run on Korean prose and `grammar-checker` on the full output?
