...
* The tension map (Stage 03), peer-review document with the missing-sixth-expert note (Stage 05), and scope document with the reader role (Stage 01).
...
2. Render `index.html` by filling the template — do not restructure it:
   * Header tokens: title, version (`V2`), date, expert count, executor line (from `executor-manifest.md`), reader role.
...
5. Present the briefing to the user with a one-line offer: add the missing sixth expert and run a V3 if they want it.
