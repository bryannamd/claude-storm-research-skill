...
2. Structure the terminal output with these sections:
   ...
   * Section 01 — the 60-second summary: the whole story in a few sentences — one or two plain orientation sentences first (what the topic is, why it matters), then the tension that matters most.
...
4. **Language, humanize & grammar-check**: before presenting, confirm the output is in the user's prompt language. For Korean runs, apply the Natural-Korean Prose Rule in `docs/output-schema.md` — translate technical terms into Korean, including research-methodology jargon (no bare transliterations such as `페인`/`익스퍼트`/`버딕트`/`코호트`; write `추적 관찰 연구`, and gloss unavoidable terms like `메타분석`/`RCT` on first use), strip translationese, and prefer short declarative sentences. If the `humanizer` and `grammar-checker` skills are installed, run `humanizer` on Korean prose sections and `grammar-checker` on the full output; if not, do an equivalent manual pass yourself. Remove AI-generated artifacts, fix spacing/punctuation, and tighten prose so the core findings stand out. Do not drop citations or verification badges.
   - **Readability pass:** run the whole report against the **Readability standard** (`docs/progress-ui.md`, echoed in `docs/output-schema.md`) — a high-school reader can follow every part (each technical term glossed in one plain line on first use), it reads smoothly start to finish, and it is economical in the caveman spirit (cut rhetoric, filler, and academic padding) even when the caveman plugin is not active. Keep it lean but grammatically complete — never squeeze out the grammar and connective words, glosses, citations, or verdict badges that make a claim followable and checkable; when brevity and clarity conflict, clarity wins.
   - **Mechanics pass (do these as literal checks, not vibes — rules in `docs/output-schema.md`):**
     1. *Word scan:* walk every noun through the high-school word test; developer-everyday traps (`PR`, `CI`, `벤치마크`, `리더보드`, `샌드박스`, `헤드리스` and their kin) count as jargon and get a first-use gloss.
     2. *Gloss placement:* no sentence carries more than one gloss; any gloss longer than a phrase becomes its own short sentence right after the term.
     3. *Orientation:* the 60-second summary opens with one or two plain sentences that tell a newcomer what the topic is and why it matters, before any number or tension.
     4. *Sentence mechanics:* body prose ends in full verb endings (no `~함`/`~음` closings outside labeled fragments), noun chains of 3+ technical terms are unrolled, and no sentence holds two dash asides.
     5. *Read-aloud test:* read the report as if aloud; rewrite any sentence you stumble over or must re-read.
5. **Code review checkpoint** — if any code changes, scripts, or tool improvements were produced during the run, route them through `codex` and `agy` (and `grok`, only when available and its experimental gate in `docs/executors.md` is confirmed) for cross-model review and QA before merging. Record the cross-model review verdict in the terminal with fields: reviewers run, fallback used (if any), findings, QA performed, pass/fail verdict, and merge decision. Code changes must not be merged until the review passes.
...
* [ ] Does the report meet the Readability standard — high-school comprehensible (jargon glossed on first use), smooth start-to-finish flow, and economical in the caveman spirit — without any comprehension-breaking compression that drops words, glosses, citations, or badges?
* [ ] Did the mechanics pass run as literal checks — word scan (developer-everyday traps glossed), gloss placement (≤1 per sentence, long glosses as their own sentence), orientation opening in the 60-second summary, verb endings in body prose, noun chains unrolled, and the read-aloud test?
...
* [ ] Were any code/tool changes reviewed by `codex` / `agy` (and `grok`, only if available and confirmed) before merging (if applicable)?
