...
Intermediate progress messages follow the standard in [docs/progress-ui.md](docs/progress-ui.md): plain language a middle/high-school student can follow (jargon glossed on first use), one progress frame per wake event during background waits, a compact position bar at every stage transition, and no fabricated time metrics. **Both the intermediate frames and the final report obey one Readability standard**, defined in `docs/progress-ui.md`. Three things at once. A high-school student can follow them, with any unfamiliar term glossed in one plain line on first use. They read smoothly, in short clear sentences that connect. And they stay lean — no filler, no academic padding — even when the caveman plugin is not active. Lean means the fluff is gone, not the grammar: never drop the words, glosses, citations, or badges that keep a sentence followable (that terse-plus-jargon squeeze is the documented past failure). The final report still follows `docs/output-schema.md` for structure, the Natural-Korean rule, and the operational readability rules (the high-school word test, gloss placement, sentence mechanics, and the read-aloud test) that make the standard checkable rather than aspirational.
