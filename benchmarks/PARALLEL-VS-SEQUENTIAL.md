...
|---|---|---|---|
| A — 5 experts (codex×3, agy×2) | 411s | **108s** | 3.82× |
...
| Phase | Sequential sum | Actual (overlapped) | Speedup |
|---|---|---|---|
| A — 5 experts (codex×3, agy×2) | 521s | **142s** | 3.66× |
...
1. **Phase A parallelization is the most reliable win** (3.66–3.82×): five independent experts collapse to the single slowest one. This barely varies by topic.
...
4. **codex is the consistent long pole** — slowest expert and slowest verification in both runs. The Rust/Go codex verification (362s) was an outlier that halved that topic's net speedup vs MCP.

## Bottleneck and further levers

**codex is the consistent long pole** — the slowest expert and the slowest verification in both topics (verification 176s on MCP, 362s on Rust/Go). Phase C synthesis (Claude) is the second fixed cost and is growing (184s on MCP). To go faster:

- **Per-expert streaming verification (biggest structural win left).** In Rust/Go the economist expert finished at 34s but verification didn't start until the 142s expert barrier; in MCP the fastest expert finished at 66s vs a 108s barrier. Kick off each verification the moment its expert returns, overlapping verification *into* Phase A. Estimated ~100–150s off the critical path on the contested topic.
- **Rebalance load off codex.** Give codex 2 experts and agy 3, and route the heavier verification cluster to agy — agy was the faster executor in both runs.
- **Split the codex verification cluster.** Instead of one codex call verifying both agy-experts, fire two smaller parallel codex calls — trades a 362s call for two concurrent ~180s calls.
...
- **Pick by goal:** raw speed on descriptive topics → sequential Claude-only is fine and simple; verification quality on contested/decision topics → parallel cross-model, where the ~2.2× parallelization keeps the slower-but-more-rigorous executors competitive on wall-clock. The next real speed cut is per-expert streaming verification.
