...
- The parallel study uses the skill's **original methodology** (codex + agy as expert/verify agents), so its tokens live on external CLI plans and aren't directly comparable to the Claude-only token column.
...
- **Parallelizing gives a stable ~2.2×** over the same calls run back-to-back (Phase A experts collapse 5→slowest; Phase B runs Claude reasoning *underneath* external verification).
...
Use **deep-research** for descriptive breadth and speed; use **storm-research** (parallel + cross-model) when the cost of a plausible-but-wrong claim justifies the slower, self-critiquing pipeline. The next real speed cut is per-expert streaming verification (start verifying each expert the moment it returns). See the two detailed reports for full per-phase data and remaining optimization levers.
